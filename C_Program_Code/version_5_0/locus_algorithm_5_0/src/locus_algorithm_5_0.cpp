					...
					long int *p_total_targets)
{
	...
	//file for the user.
	error = write_output_fits (p_fp_out_fits, p_target_star, best_intercept, colour_number, p_total_targets);
	error = error_checking (error);
	if (error != ERR_NO_ERROR)
	{
		return (error);
...

{
	...
	*/
	//declares an index for control of writing and 
	//column numbers the array is to write to
	long int row_index =(*p_total_targets) +1;
	int new_ra_col = 1;
	int new_dec_col = 2;
	int new_mag_col = 3;
	int new_av_ra_col = 4;
	...
	fits_insert_rows(*p_fp_out_fits, *p_total_targets, 1, p_status);
	...
	*/
	...
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_ra_col, row_index, 1, 1, &((*p_target)->ra), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
	...
	printf ("%lf\t", (*p_target)->ra);
		
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_dec_col, row_index, 1, 1, &((*p_target)->dec), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
	...
	printf ("%lf\t", (*p_target)->dec);
	
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_mag_col, row_index, 1, NUM_COLOURS, &((*p_target)->mag), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
	...
	printf ("%lf\t",(*p_target)->mag[colour_number]);
	
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_av_ra_col, row_index, 1, 1, &(average_point->ra), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
	...
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_av_dec_col, row_index, 1, 1, &(average_point->dec), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
	...
	fits_write_col (*p_fp_out_fits, TDOUBLE, new_av_rating_col, row_index, 1, 1, &(average_point->score), p_status);
	if (*p_status!=0)
	{
		if(*p_status == 202)
		{
