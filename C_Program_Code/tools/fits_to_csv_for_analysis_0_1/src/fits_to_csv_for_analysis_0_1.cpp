//============================================================================
// Name        : fits_to_csv_for_analysis.cpp
...

{
	...
	char t_mag_err [NAME_SIZE+1] = 		"counts_modelErr ";
	...
	for (col_num_index = 0; (col_num_index < *p_num_columns) && (found == 0); col_num_index ++)
	{
		...
		do// (name_index = 0; (name_index < NAME_SIZE) && (t_mag_err[name_index]!= '\0'); name_index ++)
		{
			if ((t_col_name[name_check_index] == t_mag_err[name_check_index]))
			{
				found = 1;
			}
			else
		...
		if (found == 1)
		{
			*p_mag_err_col = (cti_info + col_num_index)->col_number;
		}
	}
	if (found == 0)
	{
	...
	else if (*p_mag_err_col > *p_num_columns)
	{
		return (ERR_INVALID_COLUMN);
	}
	else
		...
		char *local_file_name*/)
{
	...
	struct star *star_array = (struct star *) malloc  (1 * sizeof (struct star));
	if (star_array == NULL)
	{
		return (ERR_OUT_OF_MEMORY);
	}
		...
		int *p_objc_type_col)
{
	...
	printf("\ntype,flag1,flag2,u,g,r,i,z,eu,eg,er,ei,ez,status,ra,dec\n");
	...
	{	
		...
		printf("%d,", sdss_objc_flags2);
		


		
		//gets mag
		fits_read_col (p_sdss_fits, 
						TDOUBLE, *
						p_mag_col, 
						row_index, 
						1, 
		...
		printf("%lf,%lf,%lf,%lf,%lf,", 	((star_array)->mag[0]),
										((star_array)->mag[1]),
										((star_array)->mag[2]),
										((star_array)->mag[3]),
										((star_array)->mag[4]));
		...
		fits_read_col (p_sdss_fits, 
						...
						NUM_COLOURS, 
						0, 
						((star_array+*p_out_num_rows)->mag_err), 
						0, 
						p_status);
		if (*p_status!=0)
		{
		...
		printf("%lf,%lf,%lf,%lf,%lf,", 	((star_array)->mag_err[0]),
										((star_array)->mag_err[1]),
										((star_array)->mag_err[2]),
										((star_array)->mag_err[3]),
										((star_array)->mag_err[4]));
