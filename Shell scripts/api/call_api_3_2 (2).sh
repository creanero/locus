...
lcg-cp --vo cosmo lfn:$COSMO_ROOT/$PROJ_ROOT/release/diagnose/diagnose_5_1 file:$PROJ_ROOT/release/diagnose/diagnose_5_1

#and makes it executable
chmod 755 $PROJ_ROOT/release/diagnose/diagnose_5_1

#copies extract executable from permanent (workspace/debug) to temporary (test)
lcg-cp --vo cosmo lfn:$COSMO_ROOT/$PROJ_ROOT/release/extract/extract_5_1 file:$PROJ_ROOT/release/extract/extract_5_1

#and makes it executable
chmod 755 $PROJ_ROOT/release/extract/extract_5_1
...
./$PROJ_ROOT/release/diagnose/diagnose_5_1  $PRM_FILE >/dev/null
DIAGNOSE_EXIT=$?

#executes extract
#and redirect its output to null
./$PROJ_ROOT/release/extract/extract_5_1  $PRM_FILE >/dev/null
EXTRACT_EXIT=$?
...
echo "Diagnose exit code ${DIAGNOSE_EXIT}"
echo "Extract exit code ${EXTRACT_EXIT}"
echo "Number of failed lgc-cp commands ${CP_FAIL_COUNT}"
echo "Number of failed lcg-cr commands ${CR_FAIL_COUNT}"


#displays the various possible failure states of the program at the end
echo "Diagnose exit code ${DIAGNOSE_EXIT}" >logfile.out
echo "Extract exit code ${EXTRACT_EXIT}" >> logfile.out
...
if [ x"${DIAGNOSE_EXIT}" != x"0" ]; then
 exit ${DIAGNOSE_EXIT}
fi
if [ x"${EXTRACT_EXIT}" != x"0" ]; then
 exit ${EXTRACT_EXIT}
fi
