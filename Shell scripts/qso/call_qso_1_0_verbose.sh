...
lcg-cp --vo cosmo lfn:$COSMO_ROOT/${LOCUS_ALGORITHM_DIR}/locus_algorithm_5_1 file:${LOCUS_ALGORITHM_DIR}/locus_algorithm_5_1

#and makes it executable
chmod 755 ${LOCUS_ALGORITHM_DIR}/locus_algorithm_5_1
...
lcg_cp_status=$?
 if [ x"${lcg_cp_status}" != x"0" ]; then
   ...
   echo "lcg-cp --vo cosmo lfn:${LFC_FITS_NAME} file:${NODE_FITS_NAME} copied successfully"
 fi
...
echo "Executing Locus Algorithm"

#executes algorithm
#and redirect its output to null
./${LOCUS_ALGORITHM_DIR}/locus_algorithm_5_1  ${PPR_FILE} ${COLOUR}
...
if [ x"${lfc_test_file_exists}" != x"0" ]; then
 ...
 if [ x"${lcg_cr_status}" != x"0" ]; then
   ...
   echo "lcg-cr  --vo cosmo -l lfn:${LFC_OUT_NAME} file:${NODE_OUT_NAME} completed successfully"
 fi
fi
