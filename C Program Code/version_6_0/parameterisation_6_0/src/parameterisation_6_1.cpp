...
int main(int argc, char *argv[]) 
{
	...
	{
		printf("\nChecking header for QSO ppr files now");
	...
	else if (*p_file_type == 'x')
	{
		...
		printf("\nChecking header for XOP ppr files now");
		
		
		if (fscanf (fp_csv, "NoFields,Field Number,ROW,REF RUN,REF RERUN,REF CAMCOL,REF FIELD,RUN,RERUN,CAMCOL,FIELD\n")!=0)
			//if (fscanf (fp_csv, "No of Fields for Quasar,RUN,RERUN,CAMCOL,FIELD,Quasar RA,Quasar DEC,u magnitude,g magnitude,r magnitude,i magnitude,z magnitude\n")!=0)

		{
			printf("\nThe CSV file does not contain the appropriate information for the XOP stage in its header.");
			return (ERR_FILE_READ);
		}
		
		do
		{
			printf("\nCreating XOP ppr files now");
					...
					FILE **p_fp_xop_prm)
{
	...
	{
		printf("You have selected PRM generation for API stage.");
	...
	{
		printf("You have selected PPR generation for QSO stage.");
		if( sscanf (p_arg_num_targets, "%ld", p_num_targets) !=1)
		{
			//asks the user for the number of files then writes it to the new file
	...
	{
		printf("You have selected PPR generation for XOP stage.");
		if( sscanf (p_arg_num_targets, "%ld", p_num_targets) !=1)
		{
			//asks the user for the number of files then writes it to the new file
			printf ("\nthe number of fields entered was invalid!\t");
					...
					bool *p_end_of_csv)
{
	...
	*p_fp_txt = fopen(txt_name, "w");
	...
	if (feof(*p_fp_csv))
	{
		field_index --; //Returns to last file to successfully write to file
		*p_end_of_csv = 1;
		rewind(*p_fp_csv);
		error = enter_telescope_info (	p_fov_size, 
										p_resolution,
						...
						bool *p_end_of_csv)
{
	...
	*p_fp_txt = fopen(txt_name, "w");
	...
	printf("\nEntering Telescope Details Now");
	error = enter_telescope_info (	p_fov_size, 
									p_resolution,
									p_max_mag_diff,
									p_max_col_diff,
						...
						char *p_fit_file_path)
{
					...
					
	{
		...
		for (file_index = 1; file_index < num_files; file_index ++)
		{
			...
			{
				printf("The csv file was not read appropriately for quasar analysis.");
				return (ERR_FILE_READ);
			}
						...
						char *p_fit_file_path)
{
	...
	//deleted as it is outside this loop.
	/*//sets up a variable to hold the number of targets in a given field
	...
	if (feof(*p_fp_csv))
		{
			return (ERR_END_OF_FILE);
	...
	else if (fscanf (*p_fp_csv, "%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
				&num_files,
				p_field_number,
				&row,
				&run,
				...
				&target_camcol,
				&target_field)==11	)
		//NoFields,Field Number,ROW,REF RUN,REF RERUN,REF CAMCOL,REF FIELD,RUN,RERUN,CAMCOL,FIELD
					
	{
		...
		for (file_index = 1; file_index < num_files; file_index ++)
		{
			...
			{
				printf("The csv file was not read appropriately for exoplanet analysis at line %ld", file_index);
				return (ERR_FILE_READ);
			}
	...
	{
		printf("\nThe csv file was not read appropriately for XOP analysis at line 0");
		return (ERR_FILE_READ);
	}
